...
//       根据该数组设置ui
        ...
        if WXApi.isWXAppInstalled(){
             ...
             setUpUI([ "scada_imageEmail", "scada_imageMes"], titleArray: ["邮件", "信息"])
        }
    ...
    func showShareView(){
        ...
        UIView.animateWithDuration(0.2) {
            ...
            self.shareView.frame.origin.y = self.screenH - (64 + self.buttonH + 50 + 5)
...
//    给动画
    ...
    func setUpUI(imageArray : NSArray,titleArray: NSArray)  {
        ...
        shareView = UIView(frame: CGRectMake(0, screenH, screenW, buttonH  + titleH + 2 * 15 + 64.0) )
        
        
         let viewItemBack  = UIView(frame: CGRectMake(10.0, 10.0, screenW - 20.0, buttonH + titleH + 2 * 15) )


            viewItemBack.userInteractionEnabled = true
            viewItemBack.layer.cornerRadius = 12.0
            viewItemBack.layer.masksToBounds = true
            viewItemBack.backgroundColor = UIColor(red: 61/255.0, green: 69/255.0, blue: 77/255.0, alpha: 1.0)
        
            self.shareView.addSubview(viewItemBack)
            let buttonCancel = UIButton()
        

        buttonCancel.backgroundColor = UIColor(red: 61/255.0, green: 69/255.0, blue: 77/255.0, alpha: 1.0)

            buttonCancel.layer.cornerRadius = 10.0
            buttonCancel.layer.masksToBounds = true
            buttonCancel.frame = CGRectMake(10.0, self.shareView.frame.size.height - 44.0, screenW - 20.0, 44.0)
            ...
            for index in 0 ..< titleArray.count
            {
                ...
                // 显示头像的view
                buttonItem = UIButton(frame:  CGRectMake(15 + x, 15 + y, buttonH, buttonH))
